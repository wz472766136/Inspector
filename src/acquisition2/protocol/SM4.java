/** copyright MyCompany */

package acquisition2.protocol;

import static com.riscure.util.HexUtils.*;

/*
Created by FengQi
2017.8.15
 */
public class SM4 {

	public int SBOX(int in) {
		int[] S_BOX = {  };
		return S_BOX[in];
	}

	public int ROL(int x, int i) {
		long tmp = (long) x & 0x0ffffffffL;
		tmp = tmp >> (32 - i);
		int out = (x << i) | ((int) tmp);
		return out;
	}

	public int LOL(int x, int i) {
		long tmp = (long) (x & 0x0ffffffffL);
		tmp = tmp >> i;
		return (int) (tmp);
	}

	public int LFunction(int in) {
		return in ^ (ROL(in, 2)) ^ (ROL(in, 10)) ^ (ROL(in, 18)) ^ (ROL(in, 24));
	}

	public int TFunction(int in) {
		int tmp;
		int out = 0;
		for (int i = 0; i < 4; i++) {
			tmp = LOL(in, i * 8) & 0x000000ff;
			tmp = SBOX(tmp);
			out = out | (tmp << (i * 8));
		}
		out = LFunction(out);
		return out;
	}

	public int FFunction(int X0, int X1, int X2, int X3, int RoundKey) {
		return X0 ^ TFunction(X1 ^ X2 ^ X3 ^ RoundKey);
	}

	public int[] Encrypt(int text_in[], int Key[]) {
		int i = 0;
		int[] X = new int[32 + 4];
		System.arraycopy(text_in, 0, X, 0, text_in.length);
		int[] Y = new int[4];

		int[] Roundkey = new int[32];
		Roundkey = RoundKeyGenerator(Key);

		for (i = 0; i < 32; i++) {
			X[i + 4] = FFunction(X[i], X[i + 1], X[i + 2], X[i + 3], Roundkey[i]);
			//System.out.format("RK[%d] = %x    X[%d] = %x\n", i, Roundkey[i], i, X[i + 4]);
		}
		Y[0] = X[35];
		Y[1] = X[34];
		Y[2] = X[33];
		Y[3] = X[32];
		return Y;
	}

	public int[] Decrypt(int text_in[], int Key[]) {
		int i = 0;
		int[] X = new int[32 + 4];
		System.arraycopy(text_in, 0, X, 0, text_in.length);
		int[] Y = new int[4];

		int[] Roundkey = new int[32];
		Roundkey = RoundKeyGenerator(Key);

		for (i = 0; i < 32; i++) {
			X[i + 4] = FFunction(X[i], X[i + 1], X[i + 2], X[i + 3], Roundkey[31 - i]);
			System.out.format("RK[%d] = %x    X[%d] = %x\n", 31 - i, Roundkey[31 - i], i, X[i + 4]);
		}
		Y[0] = X[35];
		Y[1] = X[34];
		Y[2] = X[33];
		Y[3] = X[32];
		return Y;
	}

	public int L2Function(int in) {
		int a = ROL(in, 13);
		int b = ROL(in, 23);
		int c = a ^ b;
		return (in ^ c);
	}

	public int T2Function(int in) {
		int tmp;
		int out = 0;
		for (int i = 0; i < 4; i++) {
			tmp = LOL(in, i * 8) & 0x000000ff;
			tmp = SBOX(tmp);
			out = out | (tmp << (i * 8));
		}
		int o = L2Function(out);
		return o;
	}

	public int[] RoundKeyGenerator(int[] Key) {
		int[] RoundKey = new int[32];
		int[] Keytemp = new int[32 + 4];
		int[] FK = { 0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC };
		int[] CK = { 0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
				0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
				0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
				0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279 };
		int i;
		for (i = 0; i < 36; i++)
			Keytemp[i] = 0;
		for (i = 0; i < 4; i++) {
			Keytemp[i] = Key[i] ^ FK[i];
		}
		for (i = 0; i < 32; i++) {
			Keytemp[i + 4] = Keytemp[i] ^ T2Function(Keytemp[i + 1] ^ Keytemp[i + 2] ^ Keytemp[i + 3] ^ CK[i]);
			RoundKey[i] = Keytemp[i + 4];
		}
		return RoundKey;
	}

	public static int[] bytesToInts(byte[] src, int offset) {
		int[] value = new int[4];
		for (int i = 0; i < 4; i++) {
			value[i] = (((src[offset + 4 * i] & 0xFF) << 24) | ((src[offset + 1 + 4 * i] & 0xFF) << 16)
					| ((src[offset + 2 + 4 * i] & 0xFF) << 8) | ((src[offset + 3 + 4 * i] & 0xFF) << 0));

		}
		return value;

	}

	public static byte[] intsToBytes(int[] value) {
		byte[] dst = new byte[16];
		for (int i = 0; i < 4; i++) {
			dst[4 * i + 0] = (byte) ((value[i] >> 24) & 0xFF);
			dst[4 * i + 1] = (byte) ((value[i] >> 16) & 0xFF);
			dst[4 * i + 2] = (byte) ((value[i] >> 8) & 0xFF);
			dst[4 * i + 3] = (byte) ((value[i] >> 0) & 0xFF);

		}
		return dst;

	}

	public static void main(String[] args) {
		byte[] CRandom = hex("1D0D57E7050816D0");
		byte[] ECRandom = rightExtend(CRandom, 16);
		byte[] TRandom = hex("1e0956e619522d35");
		byte[] ETRandom = rightExtend(TRandom, 16);
		int[] text = bytesToInts(ECRandom, 0);
		int[] Key = { 0x414A4341, 0x35383034, 0x35363030, 0x35363731 };
		int[] result = new int[4];
		SM4 sm4Engine = new SM4();
		result = sm4Engine.Encrypt(text, Key);
		for (int i = 0; i < 4; i++) {
			System.out.format("%x ", result[i]);
		}
		byte[] tmpdata = new byte[16];
		tmpdata = intsToBytes(sm4Engine.Encrypt(bytesToInts(ETRandom, 0), result));
		byte[] sdata00 = new byte[8];
		for (int i = 0; i < 8; i++) {
			sdata00[i] = (byte) (tmpdata[i] ^ tmpdata[i + 8]);
			System.out.format("%x ", sdata00[i]);
		}

	}

}
